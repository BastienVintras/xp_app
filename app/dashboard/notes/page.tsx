...
const data = await getAllNotes(user?.id as string)
  ...
  return (
    <section className="grid- items-start gap-y-8">
      <div className="flex items-center md:justify-between flex-col md:flex-row px-2">
        <div className="grid gap-1">
          <h2 className="text-3xl uppercase font-black">Notes</h2>
          <p className="text-lg text-muted-foreground">Ne perdez pas vos idées, prenez des notes</p>
      ...
      {data.length < 1 ? (
        <div className="flex min-h-[400px] flex-col items-center justify-center rounded-md border border-dashed p-3">
          ...
          <p className="text-muted-foreground text-sm">Commencez dès maintenant à céer des notes via notre application</p>
          <Button className="bg-orange-500 hover:bg-orange-600 text-white mt-4">
            <Link href="/dashboard/notes/create">Céer une nouvelle note</Link>
          </Button>
